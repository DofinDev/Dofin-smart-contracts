...
contract BoostedBunkerFactory is BasicContract {
    ...
    function createBoostedBunker (uint256[2] memory _uints, address[4] memory _addrs, string memory _name, string memory _symbol, uint8 _decimals) external onlyOwner returns(address) {
    ...
    function setTagAllBoostedBunker (bool _tag) external onlyOwner returns(bool) {
    ...
    function getTotalAssetsAllBoostedBunker () external view returns(uint256[] memory) {
